...
export const POST: APIRoute = async ({ request, redirect }) => {
  ...
  const designaciones1 = [];
    ...
    while (formData.has(`p1_id_${index}`)) {
        ...
        const oficial_mesa = formData.get(`p1_oficial_mesa_${index}`);
    ...
    while (formData.has(`p2_id_${index}`)) {
        ...
        const oficial_mesa = formData.get(`p2_oficial_mesa_${index}`);
        ...
        const { error: partidoError } = await supabaseAdmin
          ...
          .update([
            { 
                ...
                oficial_mesa_1: partido.oficial_mesa,
                oficial_mesa_2: partido.oficial_mesa_2,
              },
          ])
